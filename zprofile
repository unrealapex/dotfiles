...
isfirsttty() {
  ...
  if [ "$(uname -s)" = "Linux" ] && [ "$(tty)" = "/dev/tty1" ]; then
    return 0
  # bsd ttys start from 0 instead of 1
  elif [ "$(uname -s)" = "FreeBSD" ] && [ "$(tty)" = "/dev/ttyv0" ] ; then
    return 0
  elif [ "$(uname -s)" = "OpenBSD" ] && [ "$(tty)" = "/dev/ttyC0" ] ; then
    return 0
  else
    return 1
  fi
...
if isfirsttty; then
  ...
  case "$answer" in
    1)
      ...
      ;;
    2)
      doas gpm -m /dev/input/mice -t imps2
      dvtm-status
      ;;
    0)
      :
    ...
    "")
      ...
      ;;
    *)
      printf "%s\n" "no valid option selected, entering console"
      ;;
  esac
fi
